 ...
 *
 * セル座標: 列車 0 の列 = x 170 / 行 r の中心 = y r*20+10。sample2 下りの行構成は
 * 行 9 = 駅 0 の発行、行 17 = 駅 4 の着行、行 18 = 番線行、行 19 = 駅 4 の発行。
...
/** セルをダブルクリックして駅時刻ダイアログを開く。 */
async function openJikokuAt(page: Page, x: number, row: number): Promise<void> {
  await page.locator('.grid-scroller').dblclick({ position: { x, y: y(row) } });
...
test('連続入力モード: Alt+T で開始 → 分 2 桁入力 → Esc 終了(2.3.4 連続入力)', async ({ page }) => {
  ...
  await page.locator('.grid-scroller').click({ position: { x: COL_TRAIN0, y: y(17) } });
  ...
  await openJikokuAt(page, COL_TRAIN0, 17);
...
test('連続 1 分修正: Ctrl+Shift+L で +1 分(移動なし)が同セルに累積する(2.3.7)', async ({
  ...
  const grid = page.locator('.grid-root');
  await page.locator('.grid-scroller').click({ position: { x: COL_TRAIN0, y: y(19) } }); // 駅 4 発
  await grid.press('Control+Shift+l');
  await grid.press('Control+Shift+l');
  // 発 780(0:13)+120 = 0:15。
  await openJikokuAt(page, COL_TRAIN0, 19);
...
test('繰上げ・繰下げ: ダイアログの時刻変更が以後の駅へ伝播する(2.3.6)', async ({ page }) => {
  ...
  await openJikokuAt(page, COL_TRAIN0, 9);
...
test('分断(Ctrl+Shift+I)→ 直通化(Ctrl+Shift+U)で列車数が +1 → -1(2.3.10)', async ({ page }) => {
  ...
  await page.locator('.grid-scroller').click({ position: { x: COL_TRAIN0, y: y(19) } });
...
test('時刻のみ貼り付け(Ctrl+Shift+V): 別列車へ時刻だけ移り列車番号は不変(2.3.9)', async ({
  ...
  const grid = page.locator('.grid-root');
  // 列車 0(001)をコピー。
  await page.locator('.grid-scroller').click({ position: { x: COL_TRAIN0, y: y(0) } });
  await grid.press('Control+c');
  // 列車 1(101)へ時刻のみ貼り付け。
  await page.locator('.grid-scroller').click({ position: { x: COL_TRAIN1, y: y(0) } });
  await grid.press('Control+Shift+v');

  // 列車 1 の駅 4 着が 001 の時刻(0:12)になり、タイトルの列車番号は 101 のまま。
  await openJikokuAt(page, COL_TRAIN1, 17);
  const dialog = page.locator('dialog.prop-dialog');
