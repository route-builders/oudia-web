...
> **実装時のスコープ調整(2026-07 実装セッション)**:
>
> - **時刻表 CSV インポート**: **対象外に変更**。原典調査で、通常時刻表 CSV インポートは
...
> **実装時のスコープ調整(2026-07 実装セッション)**:
>
> - **在線表**: 単独駅のみ実装。分岐環状駅の在線表・補助列車線(ChakuOperation -2/-4)・
>   出区 ○/入区 △ 運用記号は**先送り**(原典調査で複雑度の大半かつ M7 運用データ依存と判明。
>   ユーザー決定)。単独駅在線表 = 番線別横太線 + 着発縦コネクタ。
> - **カスタマイズ時刻表**: 駅表示設定モードの一括サイクル(下り/上り各表示設定)を実装。
>   **1 列複数列車チェーンの冊子風レイアウト(customize グリッド)+ カスタマイズ時刻表 CSV は
>   先送り**。原典 operationConnectLight のチェーン構築(一本化/解結/併合)は 次列車接続作業
>   (AOperation_Junction)データに依存し、これは enableOperation<2 ではほぼ存在しない(M7 で
>   編集・生成)。チェーン無しの customize グリッド/CSV は通常時刻表とほぼ同一になるため、
>   M7 の運用データと同時に実装する方が忠実。

**完了条件**:

1. 番線増減 → 再マップのプロパティテスト green(M5 の常設テストに番線系を追加)。
2. 在線表付きフィクスチャの表示が Windows 版と目視一致し、ラウンドトリップがバイト一致を維持(単独駅スコープ)。
3. カスタマイズ時刻表の代表フィクスチャが Windows 版とセル単位で一致(スナップショット比較表を docs に残す)。**← チェーングリッド先送りに伴い M7 隣接タスクへ移動**。
...
> **実装時のスコープ調整(2026-07 実装セッション)**:
>
> - **M7e(M7d の残)を実装済み**: 箱ダイヤ形式の運用表(ビューモデル + 表示 + CSV)、
>   運用一覧図の Canvas 化(横軸ズーム・時間目盛 8 段階・ラベル重なり回避)、
>   運用表 CSV の抽出条件ダイアログ、運用探索の Worker 退避(Comlink)、F5 手動更新、
>   ダイヤグラムの運用記号(単独駅)。
> - **箱ダイヤの軸を訂正**: 原典の箱ダイヤは「**駅 = 横(列)/ 列車 = 縦(行)**」で、
>   1 列車 = 着行 / 線行(3px)/ 発行 の 3 段。設計初期の「駅 = 縦軸」は誤り。
> - **原典バグを忠実移植**: 運用表 CSV の照合方法コンボは index0「と一致する」が実装は
>   **部分一致**、index1「を含む」が **完全一致**(ラベルと実装が入れ替わっている)。
>   Windows 版と抽出結果を揃えるため、この取り違えごと移植した。
> - **未対応(後続)**: 在線表の分岐環状対応と補助列車線(chaku/hatsuOperation の -2/-4)、
>   ダイヤグラム運用記号への**探索割付運番(#2/#3)**の反映(現在は永続運番 #1 で近似)、
>   カスタマイズ時刻表のチェーングリッド + CSV、印刷ページ分割。
